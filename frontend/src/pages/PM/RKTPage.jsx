...
export default function RKTPage({ setHasPending }) {
    ...
    const getStatus = (item) => {
        const lastPm = item.tr_pm?.length ? item.tr_pm[item.tr_pm.length - 1] : null;
        ...
        <div className="pmrkt-container">
            <div className="pmrkt-header">
                ...
                <div className="pmrkt-header-actions">
                    <button className="pmrkt-reset-btn" onClick={() => { setSearch(""); fetchData(""); }} >
                        Reset
                    </button>
                    <div className="pmrkt-search-group">
                        <input className="pmrkt-search-input" type="text" placeholder="Cari..." value={search} onKeyDown={handleKeyDown} onChange={(e) => handleSearch(e.target.value)} />
                        <button className="pmrkt-search-btn" onClick={() => { setCurrentPage(1); fetchData(search); }} >
                            Search
                        </button>
                    </div>
                </div>
                        ...
                        <tbody>
                            {currentData.map((item) => (
                                <tr key={item.ID_PROGRAM_KERJA}>
                                    ...
                                    <td>Rp{" "}{Number(item.TOTAL_PROGKER || 0).toLocaleString("id-ID")}</td>
                                    <td>
                                        {(() => {
                                            const status =getStatus(item);
                                            return (
                                                <span className={`pmrkt-status ${status.className}`}>
                                                    {status.label}
                                                </span>
                                            );
                                        })()}
                                    ...
                                    <td>
                                        <button className="pmrkt-detail-btn" onClick={() => handleDetail(item.ID_PROGRAM_KERJA)}>
                                            Detail
                                        </button>
                                    </td>
                                </tr>
